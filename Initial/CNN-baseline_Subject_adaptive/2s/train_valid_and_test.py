...
def finetune_model(eegdata, eeglabel, sb, patience=10):
    ...
    if cfg.model_name == 'CNN_baseline':
        ...
        print("CNN_baseline: Freezing conv_layer, only fine-tuning FC layers")
        
    elif cfg.model_name == 'CNN_2D':
        for param in model.conv1.parameters():
            param.requires_grad = False
        print("CNN_2D: Freezing conv1, only fine-tuning FC layers")
        
    elif cfg.model_name == 'DenseNet_37':
        for name, param in model.named_parameters():
            if 'linear' not in name:  # 只训练最后的 linear 层
                param.requires_grad = False
        print("DenseNet_37: Freezing all layers except last linear layer")
...
# ----------------------initial model------------------------
    ...
    test_loader = DataLoader(dataset=test_dataset, batch_size=cfg.batch_size//8, shuffle=False)
# -------------------------test--------------------------------------------
