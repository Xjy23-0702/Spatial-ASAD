...
def train_valid_model(eegdata, eeglabel, sb, test_id):
...
# ---------------------train and valid-----------
    ...
    for epoch in range(cfg.epoch_num):
        ...
        print(f"sb: {sb}, testid: {test_id} epoch: {epoch},\n"
        ...
        if valid_loss_min > valid_loss / iter:
            ...
            savedir = os.path.join(cfg.model_dir, 'sb' + str(sb))
            if not os.path.exists(savedir):
                os.makedirs(savedir)
            saveckpt = os.path.join(savedir, 'testid' + str(test_id) + '.ckpt')
            torch.save(model.state_dict(), saveckpt)




def test_model(eegdata, eeglabel, sb, test_id):
...
# -------------------------test--------------------------------------------
    ...
    print('Subject %d Testid %d test accuracy: %.3f %%' % (sb, test_id, res))
