...
from model import CNN_CSP_Riemannian, CNN_CovarianceEnhanced
import tqdm
import torch
...
# ----------------------initial model------------------------
    valid_loss_min = 100
    if cfg.model_name == 'CNN_CSP_Riemannian':
        model = CNN_CSP_Riemannian(
            n_channels=64,
            n_csp_filters=cfg.csp_n_filters
        ).to(cfg.device)
    elif cfg.model_name == 'CNN_CovarianceEnhanced':
        model = CNN_CovarianceEnhanced(n_channels=64).to(cfg.device)
...
def finetune_model(eegdata, eeglabel, sb, patience=10):
    ...
    if cfg.model_name == 'CNN_CSP_Riemannian':
        for param in model.model.csp_layer.parameters():
            param.requires_grad = False
    elif cfg.model_name == 'CNN_CovarianceEnhanced':
        for param in model.model.conv_layer.parameters():
            param.requires_grad = False
